...
- 1차적으로 선정한 변수
  - AVG(타율) R(득점) H(안타) HR(홈런) TB(총루타) RBI(타점) SF (희생플라이) BB(4구) IBB(고의 4구) HPB(사구) OBP(출루율) SLG(장타율) OPS(출루율+장타율) SB(도루허용) SV(세이브) HLD(홀드)

### 4.2. 추가적으로 제거할 설명변수 선정
- AVG(타율), SLG(장타율), 0BP(출루율) '&larr;' 0PS(장타율+출루율)와 중복되는 변수이므로 제거
- H(안타)와 HR(홈런) '&larr;' TB(총루타) 변수가 포함하고 있으므로 제거
- R(득점)과 RBI(타점)는 중복되는 변수 '&larr;' RBI 제거 (선행연구 참고)

- 추가 제거 후 선정된 변수 리스트
  - R(득점) TB(총루타) RBI(타점) SF(희생플라이) BB(4구) IBB(고의4구) HPB(사구) OPS(출루율+장타율) SB(도루허용) SV(세이브) HLD(홀드)
